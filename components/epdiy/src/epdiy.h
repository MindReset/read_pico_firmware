...
/**
 * 差分刷新（`MODE_PACKING_1PPB_DIFFERENCE`）时跳过前导的「全屏保持」相位。
 *
 * 原厂 GC16 表里所有 (from, to) 的序列都右对齐到同一相结束，前面用保持相补齐，
 * 早期几相只有极少数转换（深色到浅灰）在动。先统计这次差分图里实际出现的
 * (from, to) 对，再从表里查这些对最早在第几相开始动，之前的相位整屏都是保持，
 * 直接不扫。像素收到的驱动序列一模一样，所以无损；省多少取决于画面内容。
 */
void epd_set_leading_skip(bool enable);

/** 最近一次 `epd_draw_base` 因上面的优化跳过的相位数。 */
int epd_last_leading_skip(void);

/**
 * 每帧开扫前先算好的行数（LCD 输出路径）。这是帧间空档的主要来源，行队列只要跑在
 * DMA 前面就够，调小能压缩每相之间的间隙；太小会欠载并置 EPD_DRAW_EMPTY_LINE_QUEUE。
 */
 ...
 * 只在这段列里算差分、标脏列，段外像素不会被驱动，回写 back_fb 也只应回写这一段。
 */
